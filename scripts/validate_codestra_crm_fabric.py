...
def validate() -> None:
    ...
    assert manifest["generic_model_proxy"] is False
    assert manifest["service_identity"] == "odoo-integration"
    assert manifest["success_authority"] == "ODOO_READBACK"
    assert not any(manifest["capabilities"].values())


if __name__ == "__main__":
    validate()
    print("ODOO_CODESTRA_CRM_FABRIC=PASS")
