...
## Controller recovery — 2026-09-06

The source controller merged in PR #55 at
`14bcc7f33dec4f56e84d79fe5fc31a77581e43b0` was removed by the later PR #62
merge. This repair restores that source controller, its policy, isolated test
runner, Git-tree validator, and regression suite against current main. Current
release and calling-contract validators remain in the source gate. Candidate
validation uses the staged tree without granting the plan job branch-write access.

Publication now requires the exact reviewed 40-character upstream SHA, rechecks
that the destination is private, and uses a credential helper rather than storing
a token in the Git remote URL. No private upstream has been read or imported by
this repair. Repository visibility, protected-environment configuration, source
copy authorization, and both credentials remain prerequisites in issue #56.
