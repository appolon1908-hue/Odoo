...
const OPEN_CALLBACK_STATES = [
    "draft", "scheduled", "reminder_pending", "ready", "due", "in_progress",
    "snoozed", "rescheduled", "missed", "escalated", "failed",
];

export class CodestraAppointmentPopouts extends Component {
    static template = "codestra_appointments.AppointmentPopouts";
    static props = [];
        ...
        try {
            if (kind === "calendar") {
                this.state.appointments = await this.orm.searchRead(
                    ...
                    ["title", "scheduled_start", "scheduled_end", "state", "customer_id", "campaign_id"],
                    { limit: 8, order: "scheduled_start asc, id asc" }
                );
            } else if (kind === "reminders") {
                this.state.reminders = await this.orm.searchRead(
                    "codestra.appointment.reminder.event",
                    [
                        ["scheduled_at", "<=", serializeDateTime(now.plus({ days: 1 }))],
                        ["state", "in", ["disabled", "scheduled", "failed"]],
                    ],
                    ["appointment_id", "event_type", "scheduled_at", "channel", "state"],
                ...
                this.state.callbacks = await this.orm.searchRead(
                    ...
                    ["reason", "scheduled_at", "state", "contact_id", "campaign_id", "priority"],
    ...
    async createAppointment() {
        ...
        await this.action.doAction({
            ...
            res_model: "codestra.call.appointment",
            views: [[false, "form"]],
            target: "new",
            context: {
                ...
                default_agent_timezone: DateTime.local().zoneName,
            },
        });
    }
    ...
    get title() {
        ...
        }[this.state.active] || "Scheduling";
    }
}

registry.category("systray").add(
    "codestra_appointments.popouts",
    { Component: CodestraAppointmentPopouts },
    { sequence: 18 }
);
