...
class AdditionalAddonReviewTests(unittest.TestCase):
    ...
    def test_additional_review_is_exact_tree_and_path_scoped(self):
        ...
        self.assertIn('codestra_observability_integration', overrides)
        self.assertEqual(set(exceptions['codestra_observability_integration']), {'models/observability.py'})
    ...
    def test_changed_subtree_invalidates_the_exception(self):
        self.document['additional_strict_overrides']['codestra_observability_integration']['current_tree'] = '0' * 40
        errors, (_, overrides, _) = self.validate(self.document)
        self.assertTrue(any('exact current tree' in error for error in errors))
        self.assertNotIn('codestra_observability_integration', overrides)
