...
""",
    ...
    def test_cursor_method_stored_in_container_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = self.write(
                Path(directory),
                "custom-addons/example/models/cursor_container.py",
                "def apply(request):\n    ops = {'sql': request.env.cr.execute}\n    ops['sql']('DELETE FROM res_users')\n",
            )
            self.assertIn(
    ...
    def test_environment_selector_stored_in_container_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = self.write(
                Path(directory),
                "custom-addons/example/controllers/environment_container.py",
                "def apply(request, payload):\n    selectors = {'model': request.env.__getitem__}\n    selectors['model'](payload['model']).sudo().create({})\n",
            )
            self.assertIn(
    ...
    def test_cursor_method_alias_stored_in_container_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = self.write(
                Path(directory),
                "custom-addons/example/models/cursor_alias_container.py",
    ...
    def test_environment_selector_alias_stored_in_container_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = self.write(
                Path(directory),
                "custom-addons/example/controllers/environment_alias_container.py",
                "def apply(request, payload):\n    select = request.env.__getitem__\n"
                "    selectors = {'model': select}\n"
                "    selectors['model'](payload['model']).sudo().create({})\n",
            )
            self.assertIn(
                "environment selector capabilities stored in containers are prohibited",
